...
from .models import Task, Project, Comment, Profile
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    progress_name = serializers.CharField(source='get_progress_display', read_only=True)
    created_at = serializers.DateTimeField(format="%Y-%m-%d %H:%M", read_only=True)
    updated_at = serializers.DateTimeField(format="%Y-%m-%d %H:%M", read_only=True)

    class Meta:
        model = Task
        fields = ['id', 'task', 'description', 'criteria', 'status', 'status_name', 'project_name',
                  'progress_name', 'progress', 'project',
...
class CommentSerializer(serializers.ModelSerializer):
    class Meta:
        model = Comment
        fields = ['id', 'text', 'user_comment', "task",'created_at','updated_at']
